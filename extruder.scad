...
module mounting(){
    translate([2,-42.5,0]) rotate([0,0,90]) cube([42,4,20]);
	difference() {
  	  difference() {
	    translate([-1,-51.5,0]) rotate([0,0,90]) cube([60,3,20]);
          translate([-5,-51.5+5,0+5+5]) rotate([0,90,0]) cylinder(h=10, r=m3_wide_radius, $fn=12);
        }
          translate([-5,0+4,0+5+5]) rotate([0,90,0]) cylinder(h=10, r=m3_wide_radius, $fn=12 );
    }
